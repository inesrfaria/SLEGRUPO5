...
export default function DashboardCopy() {
  ...
  const renderDeprivationView = () => {
    const data: any[] = [
      { name: "Total", ...Object.fromEntries(activeData.deprivation.total.map(d => [d.year.toString(), d.value])) },
      { name: "Feminino", ...Object.fromEntries(activeData.deprivation.female.map(d => [d.year.toString(), d.value])) },
      { name: "Masculino", ...Object.fromEntries(activeData.deprivation.male.map(d => [d.year.toString(), d.value])) },
    ];

    const ageData: any[] = [
      { name: "16-24", ...Object.fromEntries(activeData.deprivation.age16_24.map(d => [d.year.toString(), d.value])) },
      { name: "25-54", ...Object.fromEntries(activeData.deprivation.age25_54.map(d => [d.year.toString(), d.value])) },
      { name: "55+", ...Object.fromEntries(activeData.deprivation.age55Plus.map(d => [d.year.toString(), d.value])) },
    ];
  ...
  const renderEducationView = () => {
    const data: any[] = [
      { name: "ISCED 0-2 (Baixa)", ...Object.fromEntries(activeData.education.low.map(d => [d.year.toString(), d.value])) },
      { name: "ISCED 3-4 (Média)", ...Object.fromEntries(activeData.education.medium.map(d => [d.year.toString(), d.value])) },
      { name: "ISCED 5-8 (Alta)", ...Object.fromEntries(activeData.education.high.map(d => [d.year.toString(), d.value])) },
    ];
  ...
  const renderIntensityView = () => {
    const data: any[] = [
      { name: "Muito Alta (IMA)", ...Object.fromEntries(activeData.intensity.high.map(d => [d.year.toString(), d.value])) },
      { name: "Média (IM)", ...Object.fromEntries(activeData.intensity.medium.map(d => [d.year.toString(), d.value])) },
    ];
